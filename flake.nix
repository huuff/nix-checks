    ...
    utils.lib.eachDefaultSystem (
      ...
      in
      {
        lib = {
          ...
          # Something specific about these:
          # * They need you to pass the used nix overlay toolchain (from oxalica) because otherwise they'll use the one in nixpkgs
          #   which is almost guaranteed not to be the same one as your project used for any mildly complex project
          # * They need to build the package with naersk so all dependencies are cleanly built into nix. Otherwise, the moment any cargo command
          #   tries to do something impure (e.g. access the network), it'll crash
          rustChecks =
            { toolchain }:
            let
